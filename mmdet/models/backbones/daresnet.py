...
class DomainAttention(nn.Module):
    def __init__(self, planes, reduction=16, nclass_list=None, fixed_block=False):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        # if not self.fixed_block and False:
        #     if cfg.block_id != 4:
...
class DABottleneck(BaseModule):
    ...
    def __init__(self,
        ...
        it is "caffe", the stride-two layer is the first 1x1 conv layer.
        """
        super(DABottleneck, self).__init__(init_cfg)
        assert style in ['pytorch', 'caffe']
        assert dcn is None or isinstance(dcn, dict)
        assert plugins is None or isinstance(plugins, list)
        if plugins is not None:
        ...
        self.domain_attention = DomainAttention(planes * self.expansion, reduction=16, nclass_list=3,
                                                fixed_block=False)
...
class DAResNet(BaseModule):
    ...
    arch_settings = {
        ...
        50: (DABottleneck, (3, 4, 6, 3)),
        101: (DABottleneck, (3, 4, 23, 3)),
        152: (DABottleneck, (3, 8, 36, 3))
    }
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(DAResNet, self).__init__(init_cfg)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(DAResNet, self).train(mode)
        self._freeze_stages()
