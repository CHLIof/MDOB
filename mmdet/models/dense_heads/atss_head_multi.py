...
class ATSSHeadMulti(AnchorHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.atss_cls = []
        for i in range(self.dataset_quantity):
            self.atss_cls.append(nn.Conv2d(
            self.feat_channels,
            self.num_anchors * self.cls_out_channels,
            self.pred_kernel_size,
            padding=pred_pad_size))

        self.atss_cls  = nn.ModuleList(self.atss_cls)
    ...
    def forward(self, x: Tuple[Tensor], datasetnum_list=None) -> Tuple[List[Tensor]]:
        ...
        Returns:
            tuple: Usually a tuple of classification scores and bbox prediction
                ...
                bbox_preds (list[Tensor]): Box energies / deltas for all scale
                    ...
                    num_anchors * 4.
        """
        return multi_apply(self.forward_single, x, self.scales, datasetnum_list)

    def forward_single(self, x: Tensor, scale: Scale, datasetnum_list=None) -> Sequence[Tensor]:
        ...
        for bs_num, datasetnum in enumerate(datasetnum_list):
            cls_score.append(self.atss_cls[datasetnum](cls_feat[bs_num].unsqueeze(0)))
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList) -> dict:
        ...
        for i in range(len(batch_data_samples)):
            datasetnum_list.append(batch_data_samples[i].dataset_num)
        # print("datanum_list:", datasetnum_list)
        # exit(0)
        # print(len(x))
        datasetnum_list_input = [datasetnum_list for i in range(len(x))]
        # print(datasetnum_list_input)
        # exit(0)

        outs = self(x, datasetnum_list_input)
