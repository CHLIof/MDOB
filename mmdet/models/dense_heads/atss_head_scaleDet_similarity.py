...
class ATSSHeadScaleDetSimilarity(AnchorHead):
    ...
    def __init__(self,
        ...
        # exit(0)
        self.cls_feature_mapping = nn.Linear(in_channels,
                                             512, bias=True)

        # self.text_embedding = torch.load("./files/NWPU_embedding.pt")
        self.text_embedding = torch.load("./files/NWPU_embedding_16classes.pt")
        ...
        # self.semantic_similarities = torch.load("./files/NWPU_embedding_similarity.pt")
        self.semantic_similarities = torch.load("./files/NWPU_embedding_similarity_16classes.pt")
    ...
    def forward_single(self, x: Tensor, scale: Scale) -> Sequence[Tensor]:
        ...
        bs, channel, h, w = cls_feature.size()
        cls_feature = cls_feature.permute(0, 2, 3, 1).reshape(-1, channel)

        cls_mapping = self.cls_feature_mapping(cls_feature)
    ...
    def loss_by_feat_single(self, anchors: Tensor, cls_score: Tensor,
        ...
        #     loss_similarity = torch.tensor(0).float().cuda()
        #
        # else:
        #     loss_similarity =  self.MSE_loss_factor * self.MSE_loss(cls_score[pos_inds], similarity_label[pos_inds])
        # # 只对正样本求相似度-----end
        ...
        return loss_cls, loss_bbox, loss_centerness, centerness_targets.sum(), loss_similarity
    ...
    def loss_by_feat(
        ...
        losses_cls, losses_bbox, loss_centerness, \
            bbox_avg_factor, loss_similarity = multi_apply(
