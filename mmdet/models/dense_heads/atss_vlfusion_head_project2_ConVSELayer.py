...
class VLFusionModule(BaseModel):
    ...
    def _init_layers(self) -> None:
        ...
        self.image_feature_projection2 = []
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            self.image_feature_projection.append(
                nn.Conv2d(in_channels=self.in_channels, out_channels=self.feat_channels,
                          kernel_size=(1, 1)))
            self.image_feature_projection2.append(SELayer(self.feat_channels, 16))
        ...
        self.image_feature_projection2 = nn.ModuleList(self.image_feature_projection2)
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        if len(set(datasetnum_list)) == 1:
            ...
            for fea_num in range(len(visual_feats)):
                fea_tmp1 = self.image_feature_projection[datasetnum](visual_feats[fea_num])
                fea_tmp2 = self.image_feature_projection2[datasetnum](visual_feats[fea_num])
                x_clone.append(fea_tmp1 + fea_tmp2)
            ...
            for fea_num in range(len(visual_feats)):
                ...
                for list_index, dataset_index in enumerate(datasetnum_list):
                    fea_tmp1 = self.image_feature_projection[dataset_index](visual_feats[fea_num][list_index, :, :, :].unsqueeze(0))
                    fea_tmp2 = self.image_feature_projection2[dataset_index](visual_feats[fea_num][list_index, :, :, :].unsqueeze(0))
                    fea.append(fea_tmp1 + fea_tmp2)
...
class ATSSVLFusionHeadProject2ConVSELayer(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射(使用conv + SELayer进行特征映射)，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射
    """
