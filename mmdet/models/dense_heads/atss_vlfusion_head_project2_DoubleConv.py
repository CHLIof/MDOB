...
class VLFusionModule(BaseModel):
    ...
    def _init_layers(self) -> None:
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            self.image_feature_projection.append(
                nn.Conv2d(in_channels=self.in_channels, out_channels=self.feat_channels,
                          kernel_size=(1, 1)))
            self.image_feature_projection2.append(nn.Conv2d(in_channels=self.feat_channels, out_channels=self.feat_channels,
                          kernel_size=(1, 1)))
        ...
        # 参数初始化
        for modules in [self.bbox_pred, self.centerness, self.image_feature_projection, self.image_feature_projection2]:
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        for fea_num in range(len(visual_feats)):
            ...
            for list_index, dataset_index in enumerate(datasetnum_list):
                fea_tmp = self.image_feature_projection[dataset_index](visual_feats[fea_num][list_index, :, :, :].unsqueeze(0))
                fea_tmp = self.image_feature_projection2[dataset_index](fea_tmp)
                fea.append(fea_tmp)
...
class ATSSVLFusionHeadProject2DoubleConv(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射(使用双层Conv映射)，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射, 进行参数初始化
    """
