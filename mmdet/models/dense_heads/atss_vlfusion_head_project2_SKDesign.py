...
class SK_design(nn.Module):
    def __init__(self, in_channels=256, M=3, r: int = 16, L: int = 32):
        super().__init__()
        ...
        self.fc = nn.Sequential(nn.Conv2d(in_channels, d, kernel_size=1, stride=1, bias=False),
                                nn.BatchNorm2d(d),
                                nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        feats = feats.view(batch_size, self.M, self.in_channels, feats.shape[2], feats.shape[3])
        # print("feats shape: ", feats.shape)
...
class VLFusionModule(BaseModel):
    ...
    def _init_layers(self) -> None:
        ...
        self.image_feature_projection = SK_design(self.in_channels, self.dataset_quantity)
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            #               kernel_size=(1, 1)))
            # self.image_feature_projection.append(SELayer(self.in_channels, 16))
...
class ATSSVLFusionHeadProject2SKDesign(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射(使用SKDeign进行特征映射)，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射
    """
