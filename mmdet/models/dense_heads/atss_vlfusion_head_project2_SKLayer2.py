...
class SKLayer2(nn.Module):
    def __init__(self, features, M: int = 3, r: int = 16, stride: int = 1, L: int = 32):
        super().__init__()
        d = max(features / r, L)
        self.M = M
        ...
        self.convs.append(nn.Sequential(
            nn.Conv2d(features, features, kernel_size=3, stride=stride, padding=1 , dilation=1,
                                        bias=True)
        ))
        self.convs.append(nn.Sequential(
            nn.Conv2d(features, features, kernel_size=3, stride=stride, padding=2, dilation=2,
                      bias=True)
        ))
        # 2.fuse
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(nn.Conv2d(features*M, features, kernel_size=1, stride=1, bias=True))
        # 3.select
        self.fcs = nn.ModuleList([])
        for i in range(M):
            self.fcs.append(
                nn.Conv2d(features, features, kernel_size=1, stride=1)
            )
...
class VLFusionModule(BaseModel):
    ...
    def _init_layers(self) -> None:
        ...
        # self.image_feature_projection = SKLayer(self.in_channels)
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            #               kernel_size=(1, 1)))
            self.image_feature_projection.append(SKLayer2(self.in_channels))
        ...
        # 参数初始化
        for modules in [self.bbox_pred, self.centerness, self.image_feature_projection]:
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        #     x_clone.append(self.image_feature_projection(visual_feats[fea_num]))
        #
        ...
        if len(set(datasetnum_list)) == 1:
            ...
            dot_product_proj_tokens = self.dot_product_projection_text[datasetnum](embedding / 2.0)
            dot_product_proj_tokens = self.dot_product_projection_text2[datasetnum](dot_product_proj_tokens)

            dot_product_proj_tokens_bias = torch.matmul(embedding, self.bias_lang[datasetnum, :]) + self.bias0[datasetnum]
...
class ATSSVLFusionHeadProject2SKLayer2(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射(使用SKlayer2进行特征映射)，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射
    """
