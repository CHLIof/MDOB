...
class VLFusionModule(BaseModel):
    ...
    def __init__(self,
        ...
        self.dataset_quantity = dataset_quantity

        # print("dataset_quantity: ",self.dataset_quantity)
        # exit(0)

        # self.lang_cfg = BertConfig.from_pretrained(lang_model_name)
        self.lang_cfg = BertConfig.from_pretrained(r"/home/ly/workspace/unifymodel/GLIP2/model/models--bert-base-uncased")
    ...
    def _init_layers(self) -> None:
        ...
        self.dataset_cls = nn.Conv2d(
            self.feat_channels, self.dataset_quantity, kernel_size=1
        )
        ...
        self.dot_product_projection_text = []
        self.dot_product_projection_text2 = []
        self.image_feature_projection = []

        for dataset_index in range(self.dataset_quantity):
            self.dot_product_projection_text.append(nn.Linear(self.lang_dim,
                                                              self.num_base_priors * self.feat_channels, bias=True))
            self.dot_product_projection_text2.append(nn.Linear(self.num_base_priors * self.feat_channels,
                                                              self.num_base_priors * self.feat_channels, bias=True))

            self.image_feature_projection.append(
                nn.Conv2d(in_channels=self.in_channels, out_channels=self.feat_channels,
                          kernel_size=(1, 1)))


        self.dot_product_projection_text = nn.ModuleList(self.dot_product_projection_text)
        self.dot_product_projection_text2 = nn.ModuleList(self.dot_product_projection_text2)
        self.image_feature_projection = nn.ModuleList(self.image_feature_projection)

        self.bias_lang = nn.Parameter(
            torch.zeros(self.dataset_quantity, self.lang_dim), requires_grad=True)
        self.bias0 = nn.Parameter(
            torch.Tensor([bias_value for num in range(self.dataset_quantity)]), requires_grad=True)

        # 参数初始化
        for modules in [self.bbox_pred, self.centerness, self.dataset_cls, self.image_feature_projection]:
        ...
        # dataset_cls 参数正交初始化
        weight = self.dataset_cls.weight

        weight = weight.squeeze(3).squeeze(2)
        u, s, v = torch.linalg.svd(weight)
        weight = v[:3, :]
        weight = weight.unsqueeze(2).unsqueeze(3)
        self.dataset_cls.weight = nn.Parameter(weight)



    def forward(self, visual_feats: Tuple[Tensor],
                language_feats: dict, datasetnum_list) -> Tuple:

        if datasetnum_list == None:
            batch_size = visual_feats[0].size(0)

            datasetnum_list = [ 0 for i in range(batch_size)]

        # print(datasetnum_list)
        # exit(0)

        x_clone = []
        for fea_num in range(len(visual_feats)):
            fea = []
            for list_index, dataset_index in enumerate(datasetnum_list):
                fea.append(self.image_feature_projection[dataset_index](visual_feats[fea_num][list_index, :, :, :].unsqueeze(0)))
                # fea.append(x[fea_num][list_index, :, :, :].unsqueeze(0) + self.image_feature_projection[dataset_index](x[fea_num][list_index, :, :, :].unsqueeze(0)))

            fea = torch.cat(fea, dim=0)
            x_clone.append(fea)
            # print(fea.size())
            # exit(0)
        visual_feats = x_clone
        ...
        dot_product_proj_tokens = []
        dot_product_proj_tokens_bias = []
        for list_index, dataset_index in enumerate(datasetnum_list):
            project = self.dot_product_projection_text[dataset_index](embedding[list_index, :, :] / 2.0)
            project = self.dot_product_projection_text2[dataset_index](project)
            dot_product_proj_tokens.append(project.unsqueeze(0))

            dot_product_proj_tokens_bias.append((torch.matmul(embedding[list_index, :, :],
                                                              self.bias_lang[dataset_index, :]) + self.bias0[
                                                     dataset_index]).unsqueeze(0))

        dot_product_proj_tokens = torch.cat(dot_product_proj_tokens, dim=0)
        dot_product_proj_tokens_bias = torch.cat(dot_product_proj_tokens_bias, dim=0)
        ...
        for i, feature in enumerate(visual_feats):
            ...
            dataset_cls_logits.append(self.dataset_cls(self.avgPool(visual)).squeeze(3).squeeze(2))
        ...
        return bbox_preds, centerness, cls_logits, dataset_cls_logits


@MODELS.register_module()
class ATSSVLFusionHeadProject2DatasetLoss2(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射, 添加dataset cls loss，其中dataset分类器的参数进行
        正交初始化，并且在训练中冻住
    """
    ...
    def forward(self, visual_feats: Tuple[Tensor],
                language_feats: dict, datasetnum_list=None) -> Tuple[Tensor]:
        """Forward function."""
        bbox_preds, centerness, cls_logits, dataset_cls_logits = self.head(visual_feats,
                                                       language_feats, datasetnum_list)
        return cls_logits, bbox_preds, centerness, dataset_cls_logits
    ...
    def loss(self, visual_feats: Tuple[Tensor], language_feats: dict,
        ...
        for data_samples in batch_data_samples:
            datasetnum = data_samples.dataset_num
            datasetnum_list.append(datasetnum)
        #
        # print(datasetnum_list)
        #
        # print(len(visual_feats))
        # print(language_feats)
        # print(visual_feats[0].size())
        #
        # exit(0)

        outs = self(visual_feats, language_feats, datasetnum_list)
        self.text_masks = language_feats['masks']
        loss_inputs = outs + (batch_gt_instances, batch_img_metas,
                              batch_gt_instances_ignore, datasetnum_list)
    ...
    def loss_by_feat(
            ...
            dataset_cls: List[Tensor],
            batch_gt_instances: InstanceList,
            batch_img_metas: List[dict],
            batch_gt_instances_ignore: OptInstanceList = None,
            datasetnum_list: List[int] = None
    ) -> dict:
        ...
        # print(dataset_cls[0].shape)
        dataset_cls_cat = torch.cat(dataset_cls, dim=0)


        dataset_cls_label = []
        for i in range(len(dataset_cls)):
            dataset_cls_label.extend(datasetnum_list)

        dataset_cls_label = torch.tensor(dataset_cls_label)
        dataset_cls_label = dataset_cls_label.to(dataset_cls_cat.device)

        dataset_cls_loss = self.dataset_loss_factor * self.dataset_loss(dataset_cls_cat, dataset_cls_label)
        # print(dataset_cls_loss)
        # exit(0)
        ...
        return dict(
            ...
            loss_datasetcls = dataset_cls_loss
        )
    ...
    def predict(self,
        ...
        # outs = self(visual_feats, language_feats, datasetnum_list)
        cls_logits, bbox_preds, centerness, dataset_cls_logits = self(visual_feats, language_feats, datasetnum_list)

        outs = (cls_logits, bbox_preds, centerness)
