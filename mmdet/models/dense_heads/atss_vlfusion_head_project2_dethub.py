...
class VLFusionModule(BaseModel):
    ...
    def _init_layers(self) -> None:
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            # self.conv_param.append(nn.Parameter(torch.ones(1, self.in_channels, self.feat_channels), requires_grad=True))
        ...
        # self.conv_param = nn.ModuleList(self.conv_param)
        self.conv_param = nn.Parameter(torch.ones(self.dataset_quantity, self.in_channels, self.feat_channels), requires_grad=True)
        self.atten = nn.MultiheadAttention(self.in_channels, 4)
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        if len(set(datasetnum_list)) == 1:

            datasetnum = datasetnum_list[0]

            conv_param = self.conv_param[datasetnum].unsqueeze(0)

            conv_param_atten, _ = self.atten(conv_param, conv_param, conv_param)
            conv_param_atten = conv_param_atten[0].unsqueeze(-1).unsqueeze(-1)
            self.image_feature_projection[datasetnum].weight = nn.Parameter(conv_param_atten,
                                                                          requires_grad=True)

            # print(conv_param.shape)
            # print(conv_param_atten.shape)

            x_clone = []
            for fea_num in range(len(visual_feats)):
                # fea = []
                # for list_index, dataset_index in enumerate(datasetnum_list):
                #     fea.append(self.image_feature_projection[dataset_index](
                #         visual_feats[fea_num][list_index, :, :, :].unsqueeze(0)))
                ...
                # print(visual_feats[fea_num].shape)
                # exit(0)
                x_clone.append(self.image_feature_projection[datasetnum](visual_feats[fea_num]))
                # print(fea.size())
                # exit(0)
            ...
            conv_param_atten, _ = self.atten(self.conv_param, self.conv_param, self.conv_param)
            conv_param_atten = conv_param_atten.unsqueeze(-1).unsqueeze(-1)
            for conv_num in range(self.dataset_quantity):
                self.image_feature_projection[conv_num].weight = nn.Parameter(conv_param_atten[conv_num], requires_grad=True)
...
class ATSSVLFusionHeadProject2Dethub(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射
    """
