...
class VLFusionModule(BaseModel):
    ...
    def _init_layers(self) -> None:
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            self.image_feature_projection.append(nn.MultiheadAttention(self.in_channels, 4))
        ...
        # 参数初始化
        for modules in [self.bbox_pred, self.centerness]:
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        for fea_num in range(len(visual_feats)):
            ...
            for list_index, dataset_index in enumerate(datasetnum_list):
                vt = visual_feats[fea_num][list_index, :, :, :]
                # print(vt.shape)
                vt = vt.permute(1, 2, 0)
                # print(vt.shape)
                # exit(0)
                # vt = vt.view(vt.shape[0], vt.shape[1], -1)
                # print(vt.shape)
                # exit(0)
                vt, _ = self.image_feature_projection[dataset_index](vt, vt, vt)
                # print(vt.shape)
                vt = vt.permute(2, 0, 1).unsqueeze(0)
                # print(vt.shape)
...
class ATSSVLFusionHeadProject2SelfAttention(ATSSHead):
    ...
    Args:
        ...
        图像特征在进行dyhead前进行映射(使用自注意力)，语言特征在进行点积前进行映射，语言特征的映射为双线性层映射, 进行参数初始化
    """
