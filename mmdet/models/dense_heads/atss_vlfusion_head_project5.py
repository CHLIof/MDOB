...
class VLFusionModule(BaseModel):
        ...
        for dataset_index in range(self.dataset_quantity):
            ...
            self.image_feature_projection.append(
                nn.Conv2d(in_channels=self.in_channels, out_channels=self.feat_channels,
                          kernel_size=(1, 1)))
            self.image_feature_projection2.append(
                nn.Conv2d(in_channels=self.feat_channels, out_channels=self.feat_channels,
                          kernel_size=(1, 1)))
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        x_clone = []
        for fea_num in range(len(dyhead_tower['visual'])):
            fea = []
            for list_index, dataset_index in enumerate(datasetnum_list):
                fea_tmp = self.image_feature_projection[dataset_index](
                    dyhead_tower['visual'][fea_num][list_index, :, :, :].unsqueeze(0))
...
class ATSSVLFusionHeadProject5(ATSSHead):
    ...
    Args:
        ...
        lang_model_name (str): Name of the language model.
            ...
            图像特征在dyhead后进行映射，语言特征在进行点积前进行映射，语言特征的映射为单线性层映射，图像特征的映射为双线性层映射
    """
