...
class VLFusionModule(BaseModel):
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        sim_text_feature = torch.matmul(embedding, embedding.transpose(-1, -2))
        # print(sim_text_feature.shape)
        #
        # print(sim_text_feature[0,0,:])
        # print(sim_text_feature[0, 1, :])
        # exit(0)

        dot_product_proj_tokens = self.dot_product_projection_text(embedding /
                                                                   2.0)

        # print(dot_product_proj_tokens.shape)
        # sim_text_feature = torch.matmul(dot_product_proj_tokens, dot_product_proj_tokens.transpose(-1, -2))
        ...
        for i, feature in enumerate(visual_feats):
            ...
            # print("dot_product_logit shape: ", dot_product_logit.shape)
            # exit(0)

        return bbox_preds, centerness, cls_logits , sim_text_feature


@MODELS.register_module()
class ATSSVLFusionHeadSimloss(ATSSHead):
    ...
    def forward(self, visual_feats: Tuple[Tensor],
        ...
        """Forward function."""
        bbox_preds, centerness, cls_logits, sim_text_feature = self.head(visual_feats,
                                                       language_feats)
        return cls_logits, bbox_preds, centerness, sim_text_feature
    ...
    def loss(self, visual_feats: Tuple[Tensor], language_feats: dict,
        ...
        # # print(language_feats[0].shape)
        # # print(language_feats[1].shape)
        # print(language_feats)
        # print(language_feats['embedded'].shape)
        # exit(0)
    ...
    def loss_by_feat(
        ...
        (anchor_list, labels_list, label_weights_list, bbox_targets_list,
         bbox_weights_list, avg_factor, labels_sim_list) = cls_reg_targets
        ...
        labels_sim = torch.cat(labels_sim_list, dim = 1)

        # print(labels_sim_list.shape)
        # exit(0)
        ...
        losses_cls, losses_bbox, loss_centerness, bbox_avg_factor, loss_sim = \
            self._loss_by_feat(
                anchors,
                cls_scores,
                bbox_preds,
    ...
    def _loss_by_feat(self, anchors: Tensor, cls_score: Tensor,
        ...
        # print("labels_sim: ", labels_sim.shape)
        # print(self.text_masks.shape)
        # exit(0)
        loss_sim = self.sim_loss(cls_score, labels_sim)
        ...
        return loss_cls, loss_bbox, loss_centerness, centerness_targets.sum(), loss_sim
    ...
    def get_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         ...
         sampling_results_list, labels_sim) = multi_apply(
        ...
        labels_sim_list = images_to_levels(labels_sim, num_level_anchors)

        # print(len(labels_sim_list))
        # exit(0)

        return (anchors_list, labels_list, label_weights_list,
                bbox_targets_list, bbox_weights_list, avg_factor, labels_sim_list)
    ...
    def _get_targets_single(self,
        ...
        labels_sim = anchors.new_full((num_valid_anchors, self.feat_channels),
                                  0,
                                  dtype=torch.float32)
        ...
        if len(pos_inds) > 0:
            ...
            # print(gt_instances)
            # print(gt_instances.positive_maps[0,:])
            # # print(gt_instances.positive_maps[2, :])
            # print(gt_instances.positive_maps.shape)
            # print(sampling_result)
            # pos_labels = gt_instances.positive_maps[
            #     sampling_result.pos_assigned_gt_inds]
            # print(pos_labels.shape)
            # print(sim_text_feature.shape)
            # print(pos_labels.sum(dim=1).unsqueeze(1).shape)
            # sim_labels = torch.matmul(pos_labels, sim_text_feature) / pos_labels.sum(dim=1).unsqueeze(1)
            # print(sim_labels.shape)
            # # labels_sim[pos_inds] = torch.matmul(pos_labels, sim_text_feature) / pos_labels.sum(dim=1)
            # print(sim_text_feature[:4,:])
            # print(sim_labels[:4, :])
            # exit(0)
            ...
            pos_labels = gt_instances.positive_maps[
                sampling_result.pos_assigned_gt_inds]
            labels[pos_inds] = pos_labels

            labels_sim[pos_inds] = torch.matmul(pos_labels, sim_text_feature) / pos_labels.sum(dim=1).unsqueeze(1)
        ...
        return (anchors, labels, label_weights, bbox_targets, bbox_weights,
                pos_inds, neg_inds, sampling_result, labels_sim)
