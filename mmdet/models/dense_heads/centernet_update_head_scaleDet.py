...
class CenterNetUpdateHead_ScaleDet(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.feature_mapping = nn.Conv2d(in_channels=in_channels, out_channels=512,
                          kernel_size=(1, 1))

        self.text_embedding = torch.load("./files/NWPU_embedding.pt")
        ...
        # self.text_projection = nn.Parameter(torch.empty(512, in_channels))
        # nn.init.normal_(self.text_projection, std=1)

        self.logit_scale = nn.Parameter(torch.ones([]) * 1)
        self.logit_scale.requires_grad_(False)
        # print(self.text_embedding.device)
        # print(self.text_embedding.dtype)
        #
    ...
    def forward_single(self, x: Tensor, scale: Scale,
        ...
        feature_after_map = self.feature_mapping(cls_feat)
        # feature_after_map = reg_feat

        # print(feature_after_map.size())
        # # # exit(0)
        # #
        ...
        # 计算视觉语言相似度--start
        # text_embedding = self.text_embedding @ self.text_projection
        ...
        bs, channel, h, w = feature_after_map.size()

        feature_after_map = feature_after_map.permute(0, 2, 3, 1).reshape(-1, channel)

        # print(feature_after_map.size())

        feature_after_map = feature_after_map / feature_after_map.norm(dim=1, keepdim=True)
        # text_embedding = text_embedding / text_embedding.norm(dim=1, keepdim=True)

        logit_scale = self.logit_scale.exp()
        logits = logit_scale * feature_after_map @ self.text_embedding.t()

        logits = logits.reshape(bs, h, w, self.cls_out_channels).permute(0, 3, 1, 2)
    ...
    def loss_by_feat(
        ...
        # print(feature_after_map[0].size())
        ...
        # print(flatten_cls_scores[0].size())
        # exit(0)
        ...
        # print(flatten_bbox_preds[0].size())
        # exit(0)

        # flatten_feature_after_map = [
        #     feature_map.permute(0, 2, 3, 1).reshape(-1, 512)
        #     for feature_map in feature_after_map
        # ]

        flatten_cls_scores = torch.cat(flatten_cls_scores)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds)
        # flatten_feature_after_map = torch.cat(flatten_feature_after_map)


        # print(flatten_cls_scores.size())
        # print(flatten_bbox_preds.size())
        # print(flatten_feature_after_map.size())
        #
        # print(flatten_feature_after_map.device)

        # flatten_feature_after_map = flatten_feature_after_map / flatten_feature_after_map.norm(dim=1, keepdim=True)
        # logit_scale = self.logit_scale.exp()
        # logits = logit_scale * flatten_feature_after_map @ self.text_embedding.t()
