...
@MODELS.register_module()
class DynamicConvDetectionHub(BaseModule):
    """Implements Dynamic Convolution.
    ...
    def __init__(self,
                 ...
                 norm_cfg: OptConfigType = dict(type='LN'),
                 init_cfg: OptConfigType = None) -> None:
        super(DynamicConvDetectionHub, self).__init__(init_cfg)
        self.in_channels = in_channels
        self.feat_channels = feat_channels
        self.out_channels_raw = out_channels
        self.input_feat_shape = input_feat_shape
        ...
        # self.num_params_out = self.out_channels * self.feat_channels
        self.dynamic_layer = nn.Linear(
            self.in_channels, self.in_channels* self.out_channels * 2)

        # print(norm_cfg)
        # exit(0)


        self.norm_in = build_norm_layer(norm_cfg, self.input_feat_shape * self.input_feat_shape)[1]
        self.norm_out = build_norm_layer(norm_cfg, self.input_feat_shape * self.input_feat_shape)[1]
    ...
    def forward(self, param_feature: Tensor, input_feature: Tensor) -> Tensor:
        ...
        # print("input_feature: ", input_feature.size())  #torch.Size([200, 256, 7, 7])
        # print("param_feature: ", param_feature.size())  #torch.Size([200, 256])
        # exit(0)

        bs, channels, h, w = input_feature.size()
        # exit(0)
        #
        # input_feature = input_feature.flatten(2).permute(2, 0, 1)
        #
        ...
        # print("input_feature: ", input_feature.size())
        #
        # print("param_feature: ", param_feature.size())
        # print(param_feature.size())

        # param_feature = param_feature.view(-1, self.num_proposals, self.in_channels)

        # print(param_feature.size())

        # param_feature = param_feature.mean(dim=0)
        # param_feature = param_feature.squeeze(0)

        # print(param_feature.size())
        #
        # exit(0)

        # param_feature = param_feature.permute(1, 0)
        # print(param_feature.size())
        # exit(0)
        ...
        param_1 = parameters[:, :self.in_channels * self.out_channels].reshape(
            bs * self.in_channels, self.out_channels, 1, 1)
        param_2 = parameters[:, self.in_channels * self.out_channels:].reshape(
            bs * self.in_channels, self.out_channels, 1, 1)

        # print("param_1: ", param_1.size())
        # print("param_2: ", param_2.size())
        # exit(0)


        # print("input_feature: ", input_feature.size())
        # print("param_in: ", param_in.size())
        # print("param_out: ", param_out.size())
        input_feature = input_feature.view(1, -1, h, w)
        ...
        # param_in has shape (num_all_proposals, in_channels, feat_channels)
        # feature has shape (num_all_proposals, H*W, feat_channels)
        features = F.conv2d(input_feature, weight=param_1, bias=None, stride=1, padding=0, dilation=1, groups=1 * bs)
        # features = torch.bmm(input_feature, param_in)
        ...
        # features = torch.bmm(features, param_out)
        # print("features: ", features.size())
        features = features.view(features.size(0), features.size(1), self.input_feat_shape, self.input_feat_shape)
        features = F.conv2d(features, weight=param_2, bias=None, stride=1, padding=0, dilation=1, groups=1 * bs)
        # print("features: ", features.size())
        # exit(0)
        features = features.view(features.size(0), features.size(1), -1)
        features = self.norm_out(features)
        ...
        features = features.view(bs, -1, self.input_feat_shape, self.input_feat_shape)
